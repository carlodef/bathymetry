...
\frame{
  ...
  Notre approche :
  \begin{enumerate}
  \item Définition des incertitudes
  \item Choix d'une fonction coût
...
\begin{frame}
  ...
  On cherche à aller sur des zones de \textcolor{red}{grande pente}:
  \begin{align*}
    c(\gamma) = -\int_{t=0}^{1} |\nabla h(\gamma(t))|^{2}  dt
  \end{align*}
...
\end{frame}
\begin{frame}
  \frametitle{Un modèle un peu plus complexe} Modèle
  d'\textcolor{red}{équation différentielle} effectif pour les
  incertitudes en $x$ et $y$ $\sigma_{x}$ et $\sigma_{y}$ :
\begin{align*}
...
Généralisation : représenter l'incertitude par une \textcolor{red}{matrice SDP} $A$. Si
on pose $G = h h^{T}$,
\begin{align*}
...
\begin{frame}
  \frametitle{Modélisation de la dérive}
\begin{itemize}
\item Sources d'incertitude : précision de l'accéléromètre, erreurs
  ...
  virgule flottante)
\item \textcolor{red}{Dérive} de l'estimation de position
...
\begin{frame}
  ...
  \item Une approche par intervalles néglige la compensation des erreurs
    et est trop pessimiste. Modèle \textcolor{red}{probabiliste} ?
...
\begin{frame}
  ...
  On \textcolor{red}{évalue $h$} pour calculer $P(h(x) = h(x_{n+1}) + \varepsilon_{h})$, on calcule $P(X_{n} + v_{n} +
  \varepsilon_{v} = x)$ par \textcolor{red}{translation} et \textcolor{red}{convolution}, on \textcolor{red}{multiplie} et on
  \textcolor{red}{normalise}. En théorie bien plus précis que l'approche par boites,
